# ThurgauPopulationData SDK test runner
...
class ThurgauPopulationDataTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ThurgauPopulationDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ThurgauPopulationDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ThurgauPopulationDataTestRunner.getenv("THURGAU_POPULATION_DATA_TEST_LIVE")
        override = ThurgauPopulationDataTestRunner.getenv("THURGAU_POPULATION_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ThurgauPopulationDataTestRunner.getenv(key)
        ...
        explain = ThurgauPopulationDataTestRunner.getenv("THURGAU_POPULATION_DATA_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ThurgauPopulationDataTestRunner._test_control is not None:
            return ThurgauPopulationDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ThurgauPopulationDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ThurgauPopulationDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ThurgauPopulationDataTestRunner.load_test_control()
...
def load_env_local():
    ThurgauPopulationDataTestRunner.load_env_local()


def env_override(m):
    return ThurgauPopulationDataTestRunner.env_override(m)


def entity_data(v):
    return ThurgauPopulationDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ThurgauPopulationDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ThurgauPopulationDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ThurgauPopulationDataTestRunner.load_test_control()


def live_client_options():
    return ThurgauPopulationDataTestRunner.live_client_options()


def live_delay_ms():
    return ThurgauPopulationDataTestRunner.live_delay_ms()
