...
from thurgaupopulationdata_sdk.thurgaupopulationdata_types import (
    PopulationData,
    PopulationDataLoadMatch,
    PopulationDataListMatch,
)
...
class PopulationDataEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PopulationDataEntity(self._client, opts)
    ...
    def data_get(self) -> PopulationData:
    ...
    def match_get(self) -> PopulationData:
    ...
    def load(self, reqmatch=None, ctrl=None) -> PopulationData:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.PopulationData().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[PopulationData]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.PopulationData().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
