...
//    only - a typed ThurgauPopulationDataContext there would make every ctx
...
final class ThurgauPopulationDataOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ThurgauPopulationDataContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new ThurgauPopulationDataSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new ThurgauPopulationDataResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new ThurgauPopulationDataError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new ThurgauPopulationDataResponse($clean['response']);
    ...
    public static function err_from_map($m): ?ThurgauPopulationDataError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ThurgauPopulationDataError(is_string($code) ? $code : '', $msg);
    }
