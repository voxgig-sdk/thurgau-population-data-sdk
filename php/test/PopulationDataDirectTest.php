...
class PopulationDataDirectTest extends TestCase
{
    public function test_direct_list_population_data(): void
    {
        $setup = population_data_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-population_data", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
    ...
    public function test_direct_load_population_data(): void
    {
        $setup = population_data_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-population_data", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
...
function population_data_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new ThurgauPopulationDataSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
