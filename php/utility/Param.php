...
// ThurgauPopulationData SDK utility: param

require_once __DIR__ . '/../core/Helpers.php';

class ThurgauPopulationDataParam
{
    public static function call(ThurgauPopulationDataContext $ctx, mixed $paramdef): mixed
    {
        $point = $ctx->point;
        $spec = $ctx->spec;
        $match_val = $ctx->match;
        ...
        $akey = '';
        if ($point) {
            $alias_map = ThurgauPopulationDataHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'alias'));
            if ($alias_map) {
