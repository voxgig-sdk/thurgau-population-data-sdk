...
```php
try {
    // load() returns the ENTITY — call data_get() for the PopulationData record (throws on error).
...
```php
require_once 'thurgaupopulationdata_sdk.php';
$client = new ThurgauPopulationDataSDK($options);
```
...
```php
$client = ThurgauPopulationDataSDK::test($testopts, $sdkopts);
```
...
```php
// list() returns an array of PopulationData records (throws on error).
...
```
php/
├── thurgaupopulationdata_sdk.php          -- Main SDK class
...
The main class (`thurgaupopulationdata_sdk.php`) exports the SDK class
