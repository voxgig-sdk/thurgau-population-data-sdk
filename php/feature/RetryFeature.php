...
// ThurgauPopulationData SDK retry feature
...
class ThurgauPopulationDataRetryFeature extends ThurgauPopulationDataBaseFeature
{
    ...
    public function init(ThurgauPopulationDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ThurgauPopulationDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ThurgauPopulationDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ThurgauPopulationDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
