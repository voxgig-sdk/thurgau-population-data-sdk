...
module ThurgauPopulationDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ThurgauPopulationDataResponse.new({})
    ...
    if fetch_err
      ...
      response = ThurgauPopulationDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ThurgauPopulationDataResponse.new(fetched)
    else
