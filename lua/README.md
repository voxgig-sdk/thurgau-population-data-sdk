# ThurgauPopulationData Lua SDK



The Lua SDK for the ThurgauPopulationData API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:PopulationData()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
Check `err` first (it is non-`nil` on failure), then use `value`:
    ...
    local population_data, err = client:PopulationData():load()
    if err then error(err) end
...
```
lua/
├── thurgau-population-data_sdk.lua    -- Main SDK module
...
The main module (`thurgau-population-data_sdk`) exports the SDK constructor
