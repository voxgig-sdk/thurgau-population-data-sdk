...
describe("PopulationDataDirect", function()
  it("should direct-list-population_data", function()
    local setup = population_data_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-population_data", setup.live and "live" or "unit")
  ...
  it("should direct-load-population_data", function()
    local setup = population_data_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-population_data", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function population_data_direct_setup(mockres)
