...
// ThurgauPopulationData SDK feature test
//
...
class FtCtrl extends ThurgauPopulationDataControl
{
...
class FtClient
{
    ...
    private ThurgauPopulationDataUtility $utility;

    public function __construct(ThurgauPopulationDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): ThurgauPopulationDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ThurgauPopulationDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ThurgauPopulationDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ThurgauPopulationDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ThurgauPopulationDataConfig::make_feature($name);
    ...
    private static function buildUrl(ThurgauPopulationDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ThurgauPopulationDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ThurgauPopulationDataResponse($fetched);
        }
    ...
    private function fail(ThurgauPopulationDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ThurgauPopulationDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
