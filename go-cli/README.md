# thurgau-population-data-cli

boru-driven command-line client **and** interactive REPL for the ThurgauPopulationData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/thurgau-population-data-cli)
make build

# 2. See usage (words, entities, env vars)
./thurgau-population-data-cli --help

# 3. Provide credentials once, via the environment
export THURGAU_POPULATION_DATA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./thurgau-population-data-cli list population_data
./thurgau-population-data-cli load 1 population_data            # {id:1} shorthand
./thurgau-population-data-cli load '{id:1}' population_data       # explicit match map

# 5. Override the API base URL for a single call
THURGAU_POPULATION_DATA_BASE=https://api.example.com ./thurgau-population-data-cli list population_data

# 6. No arguments -> interactive REPL
./thurgau-population-data-cli
thurgau-population-data> list population_data
thurgau-population-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/thurgau-population-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/thurgau-population-data-cli list population_data
   ```
...
```sh
./thurgau-population-data-cli load 1 population_data          # scalar shorthand for {id:1}
./thurgau-population-data-cli load '{id:1}' population_data     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `thurgau-population-data>`). Each line is
evaluated as its own boru expression:

```text
$ ./thurgau-population-data-cli
thurgau-population-data> list population_data
thurgau-population-data> /help
thurgau-population-data> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `population_data`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/thurgau-population-data-cli`. |
...
(`./thurgau-population-data-cli <expr>`) and interactively (the REPL), and expressions compose the
