...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("THURGAUPOPULATIONDATA_TEST_LIVE") == "TRUE" ||
		os.Getenv("THURGAUPOPULATIONDATA_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("THURGAUPOPULATIONDATA_TEST_EXPLAIN"); explain != "" {
