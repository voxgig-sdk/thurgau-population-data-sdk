...
import (
	...
	omni "github.com/voxgig-sdk/thurgau-population-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ThurgauPopulationDataError); is {
				out := map[string]any{
					"name":    "ThurgauPopulationDataError",
...
func omniCtx(arg any, client *sdk.ThurgauPopulationDataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ThurgauPopulationDataSDK); is {
			client = live
