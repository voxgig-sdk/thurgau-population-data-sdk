...
//     (\`new ThurgauPopulationDataSDK(...)\` / \`ThurgauPopulationDataSDK.test(...)\` become
//     \`ThurgauPopulationDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ThurgauPopulationDataSDK'
...
const TEST_SEED = {"entity":{"population_data":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ThurgauPopulationDataSDK(...)\` and any
// \`ThurgauPopulationDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ThurgauPopulationDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ThurgauPopulationDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
