...
[MCP](https://modelcontextprotocol.io) server exposing the ThurgauPopulationData SDK as
two agent tools — `thurgau-population-data_list` and `thurgau-population-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/thurgau-population-data-mcp)
make build

# 2. Provide credentials via the environment
export THURGAU_POPULATION_DATA_APIKEY=sk_live_xxx
...
```jsonc
// thurgau-population-data_list: first page of records
{ "entity": "population_data" }
{ "entity": "population_data", "query": { } }

// thurgau-population-data_load: one record by id
{ "entity": "population_data", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/thurgau-population-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user thurgau-population-data \
     -- "$PWD"/dist/*/thurgau-population-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `thurgau-population-data_list` and `thurgau-population-data_load` tools now appear
   in new sessions. Ask the agent to *"list population_data using thurgau-population-data"*
   and it calls `thurgau-population-data_list` with `{"entity":"population_data"}`.
...
```sh
export THURGAU_POPULATION_DATA_APIKEY=sk_live_xxx            # API key
export THURGAU_POPULATION_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `thurgau-population-data_list` tool
...
### Call the `thurgau-population-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `thurgau-population-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `thurgau-population-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `THURGAU_POPULATION_DATA_APIKEY` | API key sent with every request. |
| `THURGAU_POPULATION_DATA_BASE` | Optional override of the API base URL. |
